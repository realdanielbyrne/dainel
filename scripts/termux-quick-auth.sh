...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/dainel/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Dainel Re-Auth" -c "After getting key, run: ssh $SERVER '~/dainel/scripts/mobile-reauth.sh'" --id clawd-auth
        ;;
    *)
