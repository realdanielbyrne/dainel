...
# Build and bundle Dainel into a minimal .app we can open.
# Outputs to dist/Dainel.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Dainel.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Dainel"
BUNDLE_ID="${BUNDLE_ID:-bot.dainel.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/dainel/dainel/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Dainel/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :DainelBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :DainelGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Dainel"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Dainel"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Dainel"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Dainel" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Dainel/Resources/Dainel.icns" "$APP_ROOT/Contents/Resources/Dainel.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Dainel/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying DainelKit resources"
MOLTBOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/DainelKit_DainelKit.bundle"
if [ -d "$MOLTBOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/DainelKit_DainelKit.bundle"
  cp -R "$MOLTBOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/DainelKit_DainelKit.bundle"
else
  echo "WARN: DainelKit resource bundle not found at $MOLTBOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Dainel"
killall -q Dainel 2>/dev/null || true
