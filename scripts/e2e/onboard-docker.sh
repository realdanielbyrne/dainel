...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/dainel-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/dainel-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/dainel-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/dainel-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    config_path="$HOME/.dainel/dainel.json"
    sessions_dir="$HOME/.dainel/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.dainel/dainel.json" <<'"'"'JSON'"'"'
{
