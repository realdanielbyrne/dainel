...
IMAGE_NAME="dainel-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="dainel-net-e2e-$$"
GW_NAME="dainel-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "DAINEL_GATEWAY_TOKEN=$TOKEN" \
	  -e "DAINEL_SKIP_CHANNELS=1" \
	  -e "DAINEL_SKIP_GMAIL_WATCHER=1" \
	  -e "DAINEL_SKIP_CRON=1" \
	  -e "DAINEL_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
