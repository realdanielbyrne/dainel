...
IMAGE_NAME="dainel-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/dainel-bin:$PATH"
  mkdir -p /tmp/dainel-bin

  cat > /tmp/dainel-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/dainel-bin/systemctl

  cat > /tmp/dainel-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/dainel-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/dainel"
  npm_entry="/tmp/npm-prefix/lib/node_modules/dainel/dainel.mjs"
  git_entry="/app/dainel.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/dainel-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/dainel-gateway.service"
    if [ ! -f "$unit_path" ]; then
