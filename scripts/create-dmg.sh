...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "Dainel")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "Dainel")
...
DMG_TEMP="$(mktemp -d /tmp/dainel-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/Dainel/Resources/Dainel.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/dainel-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/dainel-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/dainel-dmg-limits.txt
