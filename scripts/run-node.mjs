...
const compiler = env.DAINEL_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.DAINEL_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.DAINEL_RUNNER_LOG === "0") return;
  process.stderr.write(`[dainel] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["dainel.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
