...
BASE_IMAGE="${BASE_IMAGE:-dainel-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-dainel-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=dainel.sandbox=1)
NOTE
