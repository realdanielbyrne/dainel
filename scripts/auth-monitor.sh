...
#
# Suggested cron: */30 * * * * /home/admin/dainel/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Dainel notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., dainel-alerts)
...
STATE_FILE="$HOME/.dainel/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Dainel if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use dainel
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Dainel to $NOTIFY_PHONE..."
            dainel send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Dainel is down. Run: ssh l36 '~/dainel/scripts/mobile-reauth.sh'" "urgent"
    exit 1
