#!/bin/bash
# Setup Dainel Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'dainel-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Dainel message: Send warning via Dainel itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/dainel-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/dainel-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/dainel-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now dainel-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status dainel-auth-monitor.timer"
echo ""
echo "Quick commands:"
