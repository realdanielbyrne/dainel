...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "DAINEL_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "dainel-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.DAINEL_STATE_DIR = path.join(home, ".dainel");
    ...
    await fs.mkdir(path.join(home, ".dainel"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.dainel${path.sep}media`);
