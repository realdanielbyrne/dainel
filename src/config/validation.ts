...
import type { DainelConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: DainelConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: DainelConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = DainelSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as DainelConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as DainelConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as DainelConfig)),
    ),
  };
}
