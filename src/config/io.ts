...
import type { DainelConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareDainelVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "DAINEL_GATEWAY_TOKEN",
  "DAINEL_GATEWAY_PASSWORD",
];
...
function coerceConfig(value: unknown): DainelConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as DainelConfig;
}
...
function stampConfigVersion(cfg: DainelConfig): DainelConfig {
...
function warnIfConfigFromFuture(cfg: DainelConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareDainelVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Dainel (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: DainelConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): DainelConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as DainelConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as DainelConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: DainelConfig) {
    clearConfigCache();
...
// module scope. `DAINEL_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.DAINEL_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.DAINEL_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): DainelConfig {
...
export async function writeConfigFile(cfg: DainelConfig): Promise<void> {
  clearConfigCache();
