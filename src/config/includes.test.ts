...
const ETC_DAINEL_DIR = path.join(ROOT_DIR, "etc", "dainel");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "dainel.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcDainelPath(...parts: string[]) {
  return path.join(ETC_DAINEL_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcDainelPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "dainel.json"))).toEqual({
      shared: true,
    });
  });
});
