...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("dainel node install"), formatCliCommand("dainel node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("dainel node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    DAINEL_LOG_PREFIX: baseEnv.DAINEL_LOG_PREFIX ?? "node",
