...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "dainel-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      DAINEL_NO_RESPAWN: "1",
      DAINEL_STATE_DIR: stateDir,
      DAINEL_SKIP_CHANNELS: "1",
      DAINEL_SKIP_GMAIL_WATCHER: "1",
      DAINEL_SKIP_CRON: "1",
      DAINEL_SKIP_BROWSER_CONTROL_SERVER: "1",
      DAINEL_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "dainel-entry-bootstrap.mjs");
