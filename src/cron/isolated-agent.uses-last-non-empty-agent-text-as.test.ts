...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "dainel-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".dainel", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<DainelConfig> = {},
): DainelConfig {
  const base: DainelConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".dainel", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
