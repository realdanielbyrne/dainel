...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { dainel?: { emoji?: string } };
    expect(parsed.dainel?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { dainel?: { events?: string[] } };
