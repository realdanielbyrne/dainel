...
export function listSignalAccountIds(cfg: DainelConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: DainelConfig): string {
...
function mergeSignalAccountConfig(cfg: DainelConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: DainelConfig): ResolvedSignalAccount[] {
