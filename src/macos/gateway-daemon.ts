...
declare const __DAINEL_VERSION__: string;

const BUNDLED_VERSION =
  (typeof __DAINEL_VERSION__ === "string" && __DAINEL_VERSION__) ||
  process.env.DAINEL_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `dainel --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.DAINEL_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ?? process.env.DAINEL_GATEWAY_BIND ?? cfg.gateway?.bind ?? "loopback";
  const bind =
    bindRaw === "loopback" ||
    bindRaw === "lan" ||
    bindRaw === "auto" ||
