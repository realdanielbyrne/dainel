...
function setSlackDmPolicy(cfg: DainelConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Dainel";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Dainel`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: DainelConfig, allowFrom: string[]): DainelConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
