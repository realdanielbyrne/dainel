...
import { discoverDainelPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { DainelPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["DAINEL_PLUGIN_CATALOG_PATHS", "DAINEL_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<DainelPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: DainelPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageDainel?: DainelPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageDainel;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.dainel ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageDainel: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverDainelPlugins({ workspaceDir: options.workspaceDir });
