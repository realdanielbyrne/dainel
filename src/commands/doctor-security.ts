...
import type { DainelConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: DainelConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("dainel security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("dainel configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("dainel config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("dainel doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("dainel config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
