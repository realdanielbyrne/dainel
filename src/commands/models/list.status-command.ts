...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("dainel models auth setup-token")}\` or \`${formatCliCommand("dainel configure")}\`.`
          : `Run \`${formatCliCommand("dainel configure")}\` or set an API key env var.`;
