...
import { readConfigFileSnapshot, type DainelConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: DainelConfig, model: string): DainelConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("dainel plugins install")}\`.`,
    );
  }
