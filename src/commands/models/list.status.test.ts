...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveDainelAgentDir: vi.fn().mockReturnValue("/tmp/dainel-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/dainel-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveDainelAgentDir: mocks.resolveDainelAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/dainel-agent/auth-profiles.json");
