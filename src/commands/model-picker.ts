...
function resolveConfiguredModelRaw(cfg: DainelConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: DainelConfig): string[] {
...
export function applyPrimaryModel(cfg: DainelConfig, model: string): DainelConfig {
...
export function applyModelAllowlist(cfg: DainelConfig, models: string[]): DainelConfig {
  const defaults = cfg.agents?.defaults;
