...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "DAINEL_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("DAINEL_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv DAINEL_GATEWAY_TOKEN");
    expect(message).not.toContain("DAINEL_GATEWAY_PASSWORD");
  });
