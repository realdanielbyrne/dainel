    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from dainel.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Dainel update wizard" : "Dainel configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: DainelConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("dainel doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.DAINEL_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.DAINEL_GATEWAY_PASSWORD,
    });
    ...
    if (opts.sections) {
      ...
      if (selected.includes("health")) {
        ...
        const token = nextConfig.gateway?.auth?.token ?? process.env.DAINEL_GATEWAY_TOKEN;
        const password = nextConfig.gateway?.auth?.password ?? process.env.DAINEL_GATEWAY_PASSWORD;
      ...
      while (true) {
        ...
        if (choice === "health") {
          ...
          const token = nextConfig.gateway?.auth?.token ?? process.env.DAINEL_GATEWAY_TOKEN;
          const password =
            nextConfig.gateway?.auth?.password ?? process.env.DAINEL_GATEWAY_PASSWORD;
          await waitForGatewayReachable({
            url: wsUrl,
            token,
            password,
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.DAINEL_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.DAINEL_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.DAINEL_GATEWAY_TOKEN;
