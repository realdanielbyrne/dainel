...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.DAINEL_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.DAINEL_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "dainel-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.DAINEL_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.DAINEL_STATE_DIR = stateDir;
    process.env.DAINEL_CONFIG_PATH = path.join(stateDir, "dainel.json");
