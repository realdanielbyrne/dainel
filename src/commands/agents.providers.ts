...
function resolveDefaultAccountId(cfg: DainelConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: DainelConfig): boolean {
...
export function summarizeBindings(cfg: DainelConfig, bindings: AgentBinding[]): string[] {
