...
beforeEach(() => {
  ...
  resolveDainelPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.DAINEL_STATE_DIR;
  originalUpdateInProgress = process.env.DAINEL_UPDATE_IN_PROGRESS;
  process.env.DAINEL_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "dainel-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.DAINEL_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.DAINEL_UPDATE_IN_PROGRESS;
  } else {
    process.env.DAINEL_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveDainelPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadDainelPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "dainel-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.DAINEL_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when DAINEL_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/dainel.json",
      exists: true,
      raw: "{}",
    ...
    process.env.DAINEL_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
