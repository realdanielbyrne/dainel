...
export function listAgentEntries(cfg: DainelConfig): AgentEntry[] {
...
function resolveAgentName(cfg: DainelConfig, agentId: string) {
...
function resolveAgentModel(cfg: DainelConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: DainelConfig): AgentSummary[] {
