...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Dainel status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("dainel security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("dainel security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("dainel status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("dainel logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("dainel status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("dainel gateway probe")}`);
  }
}
