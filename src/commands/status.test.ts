...
beforeAll(() => {
  previousProfile = process.env.DAINEL_PROFILE;
  process.env.DAINEL_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.DAINEL_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/moltbot-root.js", () => ({
  resolveDainelPackageRoot: vi.fn().mockResolvedValue("/tmp/dainel"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/dainel/pnpm-lock.yaml",
      markerPath: "/tmp/dainel/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Dainel status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("dainel status --all") ||
          l.includes("dainel --profile isolated status --all") ||
          l.includes("dainel status --all") ||
          l.includes("dainel --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.DAINEL_GATEWAY_TOKEN = "abcd1234";
    try {
