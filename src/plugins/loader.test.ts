...
import { loadDainelPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.DAINEL_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `dainel-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "dainel.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.DAINEL_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadDainelPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.DAINEL_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadDainelPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadDainelPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
