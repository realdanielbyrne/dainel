...
import { discoverDainelPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: DainelPluginDefinition;
  register?: DainelPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as DainelPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as DainelPluginDefinition;
...
export function loadDainelPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverDainelPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            "dainel/plugin-sdk": pluginSdkAlias,
          },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: DainelPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as DainelPluginModule;
    } catch (err) {
