...
describe("browser config", () => {
  ...
  it("derives default ports from DAINEL_GATEWAY_PORT when unset", () => {
    const prev = process.env.DAINEL_GATEWAY_PORT;
    process.env.DAINEL_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      if (prev === undefined) {
        ...
        process.env.DAINEL_GATEWAY_PORT = prev;
      }
    }
  });
