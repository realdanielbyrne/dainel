import type { DainelConfig } from "../config/types.js";
...
export function isCommandEnabled(cfg: DainelConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: DainelConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: DainelConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: DainelConfig) {
