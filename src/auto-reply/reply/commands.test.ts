...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "dainel-commands-"));
...
function buildParams(commandBody: string, cfg: DainelConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
