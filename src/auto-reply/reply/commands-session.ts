...
import { scheduleGatewaySigusr1Restart, triggerDainelRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting dainel in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerDainelRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting dainel via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
