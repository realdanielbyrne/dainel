...
type AgentConfig = Partial<NonNullable<NonNullable<DainelConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.dainel/sessions/<SessionId>.jsonl)
...
const formatVoiceModeLine = (config?: DainelConfig, sessionEntry?: SessionEntry): string | null => {
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 Dainel ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: DainelConfig): string {
  const lines = ["ℹ️ Help", ""];
