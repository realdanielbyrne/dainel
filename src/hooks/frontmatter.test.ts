...
`;
    ...
    expect(parsed.dainel.emoji).toBe("💾");
    expect(parsed.dainel.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.dainel.emoji).toBe("📝");
    expect(parsed.dainel.events).toEqual(["command"]);
    expect(parsed.dainel.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.dainel.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"dainel": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"dainel": {"events": ["test"]}}');
  });
...
describe("resolveDainelMetadata", () => {
  it("extracts dainel metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveDainelMetadata(frontmatter);
  ...
  it("returns undefined when dainel key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        dainel: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Dainel" },
            { id: "npm", kind: "npm", package: "@dainel/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@dainel/hook");
  });
    ...
    "dainel":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Dainel" }],
      },
  }
---
...
`;
    ...
    const dainel = resolveDainelMetadata(frontmatter);
    expect(dainel).toBeDefined();
    expect(dainel?.emoji).toBe("💾");
    expect(dainel?.events).toEqual(["command:new"]);
    expect(dainel?.requires?.config).toEqual(["workspace.dir"]);
    expect(dainel?.install?.[0].kind).toBe("bundled");
  });
