...
function parseDnsSdBrowse(stdout: string): string[] {
  ...
  for (const raw of stdout.split("\n")) {
    const line = raw.trim();
    if (!line || !line.includes("_dainel-gw._tcp")) continue;
    if (!line.includes("Add")) continue;
    const match = line.match(/_dainel-gw\._tcp\.?\s+(.+)$/);
    if (match?.[1]) {
...
async function discoverViaDnsSd(
  ...
  const browse = await run(["dns-sd", "-B", "_dainel-gw._tcp", domain], {
    timeoutMs,
  });
  const instances = parseDnsSdBrowse(browse.stdout);
  const results: GatewayBonjourBeacon[] = [];
  for (const instance of instances) {
    const resolved = await run(["dns-sd", "-L", instance, "_dainel-gw._tcp", domain], {
      timeoutMs,
    });
...
async function discoverWideAreaViaTailnetDns(
  ...
  const probeName = `_dainel-gw._tcp.${domain.replace(/\.$/, "")}`;
  ...
  for (const ptr of ptrs) {
    ...
    if (!ptrName) continue;
    const instanceName = ptrName.replace(/\.?_dainel-gw\._tcp\..*$/, "");
...
function parseAvahiBrowse(stdout: string): GatewayBonjourBeacon[] {
  ...
  for (const raw of stdout.split("\n")) {
    ...
    if (!line) continue;
    if (line.startsWith("=") && line.includes("_dainel-gw._tcp")) {
      if (current) results.push(current);
      const marker = " _dainel-gw._tcp";
...
async function discoverViaAvahi(
  ...
  const args = ["avahi-browse", "-rt", "_dainel-gw._tcp"];
