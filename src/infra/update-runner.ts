...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "dainel-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "dainel doctor",
        managerScriptArgs(manager, "dainel", ["doctor", "--non-interactive"]),
        gitRoot,
        { DAINEL_UPDATE_IN_PROGRESS: "1" },
      ),
    );
