...
export const WIDE_AREA_DISCOVERY_DOMAIN = "dainel.internal.";
export const WIDE_AREA_ZONE_FILENAME = "dainel.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*dainel-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "dainel";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "dainel");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "dainel-gw");
  ...
  records.push(`_dainel-gw._tcp IN PTR ${instanceLabel}._dainel-gw._tcp`);
  records.push(`${instanceLabel}._dainel-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._dainel-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; dainel-content-hash: ${contentHash}\n${contentBody}`;
}
