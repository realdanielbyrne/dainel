...
function candidateBinDirs(opts: EnsureDainelPathOpts): string[] {
  ...
  // Bundled macOS app: `dainel` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingDainel = path.join(execDir, "dainel");
    if (isExecutable(siblingDainel)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/dainel` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "dainel"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `dainel` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureDainelCliOnPath(opts: EnsureDainelPathOpts = {}) {
  if (isTruthyEnvValue(process.env.DAINEL_PATH_BOOTSTRAPPED)) return;
  process.env.DAINEL_PATH_BOOTSTRAPPED = "1";
