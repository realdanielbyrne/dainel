...
export function triggerDainelRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.DAINEL_SYSTEMD_UNIT,
        process.env.DAINEL_PROFILE,
      );
  ...
  const label =
    process.env.DAINEL_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.DAINEL_PROFILE);
