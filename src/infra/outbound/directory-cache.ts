...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: DainelConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: DainelConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: DainelConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: DainelConfig): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: DainelConfig): void {
