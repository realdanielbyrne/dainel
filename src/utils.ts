...
export function resolveConfigDir(
  ...
  const override = env.DAINEL_STATE_DIR?.trim() || env.DAINEL_STATE_DIR?.trim();
  if (override) return resolveUserPath(override);
  const legacyDir = path.join(homedir(), ".dainel");
  const newDir = path.join(homedir(), ".dainel");
  try {
...
// Configuration root; can be overridden via DAINEL_STATE_DIR.
