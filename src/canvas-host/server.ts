...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.dainelCanvasA2UIAction ||
        window.webkit.messageHandlers.dainelCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.dainelCanvasA2UIAction &&
        typeof window.dainelCanvasA2UIAction.postMessage === "function") ||
      (window.dainelCanvasA2UIAction &&
        typeof window.dainelCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.dainelSendUserAction === "function" ? window.dainelSendUserAction : undefined;
  if (!window.dainelSendUserAction && legacySend) {
    window.dainelSendUserAction = legacySend;
  }
  if (!window.dainelSendUserAction && typeof window.dainelSendUserAction === "function") {
    window.dainelSendUserAction = window.dainelSendUserAction;
  }
  const hasHelper = () =>
    typeof window.dainelSendUserAction === "function" ||
    typeof window.dainelSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("dainel:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Dainel node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.dainelSendUserAction === "function"
        ? window.dainelSendUserAction
        : window.dainelSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.DAINEL_SKIP_CANVAS_HOST)) return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Dainel Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
