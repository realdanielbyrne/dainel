...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-dainel-message-channel") ?? "");
  const accountId = getHeader(req, "x-dainel-account-id")?.trim() || undefined;
  ...
  const allTools = createDainelTools({
