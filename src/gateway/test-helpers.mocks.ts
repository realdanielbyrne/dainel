...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `dainel-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.DAINEL_CONFIG_PATH = path.join(root, "dainel.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "dainel.json");
