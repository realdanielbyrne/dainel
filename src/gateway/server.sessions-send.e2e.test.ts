...
import { createDainelTools } from "../agents/moltbot-tools.js";
...
beforeAll(async () => {
  prevGatewayPort = process.env.DAINEL_GATEWAY_PORT;
  prevGatewayToken = process.env.DAINEL_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.DAINEL_GATEWAY_PORT = String(gatewayPort);
  process.env.DAINEL_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayPort === undefined) {
    ...
    process.env.DAINEL_GATEWAY_PORT = prevGatewayPort;
  }
  if (prevGatewayToken === undefined) {
    delete process.env.DAINEL_GATEWAY_TOKEN;
  } else {
    process.env.DAINEL_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createDainelTools().find((candidate) => candidate.name === "sessions_send");
