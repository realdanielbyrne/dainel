...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Dainel";
  if (/dainel/i.test(trimmed)) return trimmed;
  return `${trimmed} (Dainel)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.DAINEL_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "dainel");
  ...
  const binCli = path.join(cwd, "bin", "dainel.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.DAINEL_TAILNET_DNS?.trim();
