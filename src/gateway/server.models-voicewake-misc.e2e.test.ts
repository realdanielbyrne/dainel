...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.DAINEL_STATE_DIR = path.join(homeDir, ".dainel");
    ...
    return () => {
      ...
      if (prevStateDir === undefined) {
        ...
        process.env.DAINEL_STATE_DIR = prevStateDir;
      }
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "dainel-home-"));
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".dainel", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const prevCanvasPort = process.env.DAINEL_CANVAS_HOST_PORT;
    process.env.DAINEL_GATEWAY_TOKEN = "secret";
    testTailnetIPv4.value = "100.64.0.1";
    testState.gatewayBind = "lan";
    const canvasPort = await getFreePort();
    testState.canvasHostPort = canvasPort;
    process.env.DAINEL_CANVAS_HOST_PORT = String(canvasPort);
    ...
    if (prevToken === undefined) {
      ...
      process.env.DAINEL_GATEWAY_TOKEN = prevToken;
    }
    if (prevCanvasPort === undefined) {
      delete process.env.DAINEL_CANVAS_HOST_PORT;
    } else {
      process.env.DAINEL_CANVAS_HOST_PORT = prevCanvasPort;
    }
  });
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.DAINEL_CONFIG_PATH;
    if (!configPath) throw new Error("Missing DAINEL_CONFIG_PATH");
