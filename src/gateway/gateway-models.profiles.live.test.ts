...
import { resolveDainelAgentDir } from "../agents/agent-paths.js";
...
import { ensureDainelModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { DainelConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.DAINEL_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.DAINEL_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.DAINEL_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<DainelConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.DAINEL_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.DAINEL_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveDainelAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "dainel-live-state-"));
  ...
  process.env.DAINEL_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.dainel-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveDainelAgentDir();
  const sanitizedCfg: DainelConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Dainel live tool probe (local, safe): " +
    ...
    process.env.DAINEL_CONFIG_PATH = previous.configPath;
    process.env.DAINEL_GATEWAY_TOKEN = previous.token;
    process.env.DAINEL_SKIP_CHANNELS = previous.skipChannels;
    process.env.DAINEL_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.DAINEL_SKIP_CRON = previous.skipCron;
    process.env.DAINEL_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.DAINEL_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.DAINEL_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureDainelModelsJson(cfg);
      ...
      const rawModels = process.env.DAINEL_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.dainel-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
