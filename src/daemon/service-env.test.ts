...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.DAINEL_GATEWAY_PORT).toBe("18789");
    expect(env.DAINEL_GATEWAY_TOKEN).toBe("secret");
    expect(env.DAINEL_SERVICE_MARKER).toBe("dainel");
    expect(env.DAINEL_SERVICE_KIND).toBe("gateway");
    expect(typeof env.DAINEL_SERVICE_VERSION).toBe("string");
    expect(env.DAINEL_SYSTEMD_UNIT).toBe("dainel-gateway.service");
    if (process.platform === "darwin") {
      expect(env.DAINEL_LAUNCHD_LABEL).toBe("bot.dainel.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", DAINEL_PROFILE: "work" },
      port: 18789,
    });
    expect(env.DAINEL_SYSTEMD_UNIT).toBe("dainel-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.DAINEL_LAUNCHD_LABEL).toBe("bot.dainel.work");
    }
  });
});
