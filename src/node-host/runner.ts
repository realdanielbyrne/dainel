...
import { ensureDainelCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.DAINEL_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed = process.env.DAINEL_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureDainelCliOnPath({ pathEnv: process.env.PATH ?? "" });
