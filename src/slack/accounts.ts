...
export function listSlackAccountIds(cfg: DainelConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: DainelConfig): string {
...
function mergeSlackAccountConfig(cfg: DainelConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: DainelConfig): ResolvedSlackAccount[] {
