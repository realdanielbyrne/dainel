...
function resolveExecConfig(cfg: DainelConfig | undefined) {
...
export function createDainelCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createDainelReadTool(freshReadTool)];
    }
