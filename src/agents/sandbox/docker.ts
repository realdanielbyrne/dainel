...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "dainel.sandbox=1");
  args.push("--label", `dainel.sessionKey=${params.scopeKey}`);
  args.push("--label", `dainel.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `dainel.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "dainel.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`dainel sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`dainel sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("dainel sandbox recreate --all");
}
