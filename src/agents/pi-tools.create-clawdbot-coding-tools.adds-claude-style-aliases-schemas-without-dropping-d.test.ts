...
const defaultTools = createDainelCodingTools();

describe("createDainelCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dainel-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createDainelCodingTools();
    ...
    try {
      ...
      const contents = "Hello from dainel read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "dainel-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "dainel-workspace"),
    ...
    const tools = createDainelCodingTools({ sandbox });
