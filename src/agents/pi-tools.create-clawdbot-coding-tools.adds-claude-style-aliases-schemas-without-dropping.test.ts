...
import { createDainelTools } from "./moltbot-tools.js";
import { __testing, createDainelCodingTools } from "./pi-tools.js";
...
describe("createDainelCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createDainelCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dainel-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "dainel-outside.txt");
