...
/**
 * Cleanup suspended Dainel CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: DainelConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dainel-cli-images-"));
  const paths: string[] = [];
