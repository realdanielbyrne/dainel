...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.DAINEL_AGENT_DIR;
    else process.env.DAINEL_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dainel-chutes-"));
    process.env.DAINEL_STATE_DIR = tempDir;
    process.env.DAINEL_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
