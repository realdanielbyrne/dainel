...
export function listGoogleChatAccountIds(cfg: DainelConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: DainelConfig): string {
...
export function listEnabledGoogleChatAccounts(cfg: DainelConfig): ResolvedGoogleChatAccount[] {
