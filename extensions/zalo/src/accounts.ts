...
export function listZaloAccountIds(cfg: DainelConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultZaloAccountId(cfg: DainelConfig): string {
...
function mergeZaloAccountConfig(cfg: DainelConfig, accountId: string): ZaloAccountConfig {
...
export function listEnabledZaloAccounts(cfg: DainelConfig): ResolvedZaloAccount[] {
