...
export function listBlueBubblesAccountIds(cfg: DainelConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultBlueBubblesAccountId(cfg: DainelConfig): string {
...
export function listEnabledBlueBubblesAccounts(cfg: DainelConfig): ResolvedBlueBubblesAccount[] {
