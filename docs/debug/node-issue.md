...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Dainel
---

# Node + tsx "__name is not a function" crash

## Summary
Running Dainel via Node with `tsx` fails at startup with:

```
[dainel] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `dainel status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch dainel.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node dainel.mjs status` works on Node 25.
