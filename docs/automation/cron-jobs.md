...
- Jobs persist under `~/.dainel/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.dainel/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.dainel/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
dainel cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
dainel cron edit <jobId> --agent ops
dainel cron edit <jobId> --clear-agent
```
```
...
```bash
dainel system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`dainel system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `DAINEL_SKIP_CRON`.
