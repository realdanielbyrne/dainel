...
Dainel features that can generate provider usage or paid API calls.
...
- `dainel status --usage` and `dainel channels list` show provider **usage windows**
...
Dainel can pick up credentials from:
...
### 8) Model scan / probe
`dainel models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
