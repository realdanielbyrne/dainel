---
summary: "Dainel on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Dainel on a Raspberry Pi
  - Running Dainel on ARM devices
  - Building a cheap always-on personal AI
---

# Dainel on Raspberry Pi

## Goal

Run a persistent, always-on Dainel Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Dainel features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled dainel

# Enable if not
sudo systemctl enable dainel

# Start on boot
sudo systemctl start dainel
```
...
```bash
# Check logs
journalctl -u dainel --no-pager -n 100

# Common fix: rebuild
cd ~/dainel  # if using hackable install
npm run build
sudo systemctl restart dainel
```
