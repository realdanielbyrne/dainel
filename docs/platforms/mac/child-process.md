...
service via the external `dainel` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`DAINEL_PROFILE`; legacy `com.dainel.*` is supported).
