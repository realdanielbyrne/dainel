---
summary: "Chrome extension: let Dainel drive your existing Chrome tab"
read_when:
...
The Dainel Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate clawd-managed Chrome profile.
...
Dainel then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Dainel release (npm package) as static files. There is no separate “build” step.

After upgrading Dainel:
- Re-run `dainel browser extension install` to refresh the installed files under your Dainel state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Dainel ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Dainel to control.
...
- `ON`: attached; Dainel can drive that tab.
...
Debugging: `dainel sandbox explain`
...
`dainel browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `dainel browser extension install` first to copy the extension to a stable location under your Dainel state directory.
