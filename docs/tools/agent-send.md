---
summary: "Direct `dainel agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
---
# `dainel agent` (direct agent runs)

`dainel agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `dainel message --target`).
...
```bash
dainel agent --to +15555550123 --message "status update"
dainel agent --agent ops --message "Summarize logs"
...
dainel agent --to +15555550123 --message "Summon reply" --deliver
