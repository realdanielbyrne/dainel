...
- **gateway host** → `dainel` process on the gateway machine
...
`~/.dainel/exec-approvals.json` directly.

CLI: `dainel approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
