...
- `/usage cost` in chats: local cost summary aggregated from Dainel session logs.
- CLI: `dainel status --usage` prints a full per-provider breakdown.
- CLI: `dainel channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
