...
- **Session store** (chat history + routing state) under `~/.dainel/agents/<agentId>/sessions`.
...
available from `~/.dainel/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
- State dir: `~/.dainel` (or `DAINEL_STATE_DIR`)
- Workspace: `~/clawd` (or `~/clawd-<agentId>`)
- Agent dir: `~/.dainel/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.dainel/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Dainel runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/clawd` (or `~/clawd-<profile>` when `DAINEL_PROFILE` is set).
- State defaults to `~/.dainel/agents/main/agent`.
...
```js
{
    ...
    list: [
      {
        ...
        agentDir: "~/.dainel/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/clawd-work",
        agentDir: "~/.dainel/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.dainel/credentials/whatsapp/personal
          // authDir: "~/.dainel/credentials/whatsapp/personal",
        },
        biz: {
