...
read_when:
  - "You installed Dainel but `dainel` is “command not found”"
...
Dainel’s runtime baseline is **Node 22+**.

If you can run `npm install -g dainel@latest` but later see `dainel: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
