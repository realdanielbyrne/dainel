---
summary: "Use Venice AI privacy-focused models in Dainel"
read_when:
  - You want privacy-focused inference in Dainel
...
```bash
dainel onboard --auth-choice venice-api-key
```
...
```bash
dainel chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Dainel shows all available Venice models. Pick based on your needs:
...
```bash
dainel models set venice/claude-opus-45
dainel models set venice/llama-3.3-70b
```

List all available models:

```bash
dainel models list | grep venice
```

## Configure via `dainel configure`

1. Run `dainel configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Dainel automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
dainel chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
dainel chat --model venice/claude-opus-45

# Use uncensored model
dainel chat --model venice/venice-uncensored

# Use vision model with image
dainel chat --model venice/qwen3-vl-235b-a22b

# Use coding model
dainel chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `dainel models list` to see currently available models. Some models may be temporarily offline.
