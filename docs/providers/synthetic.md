---
summary: "Use Synthetic's Anthropic-compatible API in Dainel"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Dainel registers it as the
...
```bash
dainel onboard --auth-choice synthetic-api-key
```
...
Note: Dainel's Anthropic client appends `/v1` to the base URL, so use
