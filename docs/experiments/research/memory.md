...
read_when:
  ...
  - Deciding: standalone CLI vs deep Dainel integration
...
Recommendation: **deep integration in Dainel**, but keep a separable core library.

### Why integrate into Dainel?
- Dainel already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `dainel memory recall "…" --k 25 --since 30d`
  - `dainel memory reflect --since 7d`
