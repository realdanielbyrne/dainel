...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `dainel sandbox explain`.
...
Legacy `agent.*` configs are migrated by `dainel doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=dainel.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${DAINEL_STATE_DIR:-$HOME/.dainel}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
