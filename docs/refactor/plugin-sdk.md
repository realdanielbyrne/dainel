...
Delivery:
- Publish as `@dainel/plugin-sdk` (or export from core under `dainel/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `DainelPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: DainelConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: DainelConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: DainelConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: DainelConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: DainelConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: DainelConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@dainel/plugin-sdk`.
- Add `api.runtime` to `DainelPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `dainelRuntime: ">=2026.2.0"`).
