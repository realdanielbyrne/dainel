...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.dainel/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.dainel/node.json` (node id + pairing token).
...
Path: `~/.dainel/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.dainel/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.dainel/exec-approvals.sock` (0600).
