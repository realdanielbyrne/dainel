...
- Replaces the legacy `gateway` command. CLI entry point: `dainel gateway`.
...
dainel gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
dainel gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.dainel/dainel.json` (or `DAINEL_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__dainel__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `DAINEL_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `DAINEL_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `DAINEL_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.dainel.*` may still exist)
- Linux: `dainel-gateway-<profile>.service`
- Windows: `Dainel Gateway (<profile>)`

Install metadata is embedded in the service config:
- `DAINEL_SERVICE_MARKER=dainel`
- `DAINEL_SERVICE_KIND=gateway`
...
```bash
dainel --dev setup
dainel --dev gateway --allow-unconfigured
...
- `DAINEL_STATE_DIR=~/.dainel-dev`
- `DAINEL_CONFIG_PATH=~/.dainel-dev/dainel.json`
- `DAINEL_GATEWAY_PORT=19001` (Gateway WS + HTTP)
...
- `canvasHost.port = base + 4` (or `DAINEL_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `DAINEL_CONFIG_PATH`
- unique `DAINEL_STATE_DIR`
...
Example:
```bash
DAINEL_CONFIG_PATH=~/.dainel/a.json DAINEL_STATE_DIR=~/.dainel-a dainel gateway --port 19001
DAINEL_CONFIG_PATH=~/.dainel/b.json DAINEL_STATE_DIR=~/.dainel-b dainel gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `dainel health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `dainel`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `dainel gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.dainel.*` is cleaned up).
  - `dainel doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Dainel profile services.
  ...
  - Cleanup: `dainel gateway uninstall` (current service) and `dainel doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `dainel gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Dainel.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.dainel.*` labels still unload cleanly).
- To stop it cleanly, use `dainel gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `dainel gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `dainel gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)
Dainel installs a **systemd user service** by default on Linux/WSL2. We
...
`dainel gateway install` writes the user unit. `dainel doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/dainel-gateway[-<profile>].service`:
```
[Unit]
Description=Dainel Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/dainel gateway --port 18789
Restart=always
RestartSec=5
Environment=DAINEL_GATEWAY_TOKEN=
...
```
systemctl --user enable --now dainel-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/dainel-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now dainel-gateway[-<profile>].service
```
...
## CLI helpers
- `dainel gateway health|status` — request health/status over the Gateway WS.
- `dainel message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `dainel agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `dainel gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `dainel gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `dainel gateway` and the legacy TCP control port.
