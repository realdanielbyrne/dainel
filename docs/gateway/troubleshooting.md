---
summary: "Quick troubleshooting guide for common Dainel failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When Dainel misbehaves, here's how to fix it.
...
|---|---|---|
| `dainel status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `dainel status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `dainel status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `dainel gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `dainel channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `dainel gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `dainel logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `dainel status --all` (it redacts tokens). If you paste `dainel status`, consider setting `DAINEL_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  dainel models auth setup-token --provider anthropic
  ```
...
```bash
dainel models auth paste-token --provider anthropic
dainel models status
```
...
**Logs:**
- Preferred: `dainel logs --follow`
- File logs (always): `/tmp/dainel/dainel-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$DAINEL_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u dainel-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  dainel config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  dainel config set gateway.mode remote
  dainel config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `dainel setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.dainel/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `dainel doctor`
...
- then run `dainel sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `dainel gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `dainel gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `dainel gateway status` and `dainel doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `DAINEL_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `dainel gateway status` shows a config mismatch**
...
- Fix: rerun `dainel gateway install --force` from the same `--profile` / `DAINEL_STATE_DIR` you want the service to use.

**If `dainel gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `dainel doctor` to update it (or `dainel gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `DAINEL_GATEWAY_TOKEN`) and restart the service.

**If `dainel gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/dainel` on disk.
...
Symptoms: `pwd` or file tools show `~/.dainel/sandboxes/...` even though you
...
Dainel intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `dainel models list` or
  `dainel models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${DAINEL_CONFIG_PATH:-$HOME/.dainel/dainel.json}"
```
...
tail -f "$(ls -t /tmp/dainel/dainel-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
dainel logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.dainel/agents/<agentId>/sessions/
```
...
dainel logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
dainel gateway --verbose
```

If you’re logged out / unlinked:

```bash
dainel channels logout
trash "${DAINEL_STATE_DIR:-$HOME/.dainel}/credentials" # if logout can't cleanly remove everything
dainel channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/dainel/dainel-*.log | head -1)" | tail -20
```

### High Memory Usage

Dainel keeps conversation history in memory.
...
Dainel now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `dainel doctor` reports every invalid entry.
- `dainel doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `dainel logs`, `dainel health`, `dainel status`, `dainel gateway status`, and `dainel gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/dainel/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `dainel models status` (CLI).
...
5) Run `dainel channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Dainel scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/dainel/dainel/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
dainel gateway status
dainel gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.dainel.* still works)
```
...
Ensure the global `dainel` CLI is installed and matches the app version:
```bash
dainel --version
npm install -g dainel@<version>
```
...
#   ${DAINEL_CONFIG_PATH:-$HOME/.dainel/dainel.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
dainel gateway --verbose
dainel channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/dainel/dainel-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$DAINEL_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.dainel/logs/...`; profiles use `~/.dainel-<profile>/logs/...`)<br />Linux: `journalctl --user -u dainel-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Dainel Gateway (<profile>)" /V /FO LIST` |
| Session files | `$DAINEL_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$DAINEL_STATE_DIR/media/` |
| Credentials | `$DAINEL_STATE_DIR/credentials/` |
...
trash "${DAINEL_STATE_DIR:-$HOME/.dainel}"
dainel channels login         # re-pair WhatsApp
dainel gateway restart           # or: dainel gateway
```
...
1. Check logs first: `/tmp/dainel/` (default: `dainel-YYYY-MM-DD.log`, or your configured `logging.file`)
