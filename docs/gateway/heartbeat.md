...
- During heartbeat runs, Dainel treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `dainel sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Dainel skips the heartbeat run entirely (no model call).
...
```bash
dainel system event --text "Check for urgent follow-ups" --mode now
```
