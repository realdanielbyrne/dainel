---
summary: "All configuration options for ~/.dainel/dainel.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Dainel reads an optional **JSON5** config from `~/.dainel/dainel.json` (comments + trailing commas allowed).

If the file is missing, Dainel uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
Dainel only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `dainel doctor`, `dainel logs`, `dainel health`, `dainel status`, `dainel service`, `dainel help`).
- Run `dainel doctor` to see the exact issues.
- Run `dainel doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `dainel config set`. Keep a backup of `~/.dainel/dainel.json`.
...
```bash
dainel gateway call config.get --params '{}' # capture payload.hash
dainel gateway call config.apply --params '{
...
```bash
dainel gateway call config.get --params '{}' # capture payload.hash
dainel gateway call config.patch --params '{
...
{ "$include": "/etc/dainel/base.json5" } // absolute
...
```json5
// ~/.dainel/clients/mueller/agents.json5
[
...
```json5
// ~/.dainel/clients/mueller/broadcast.json5
{
...
Dainel reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Dainel runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `DAINEL_LOAD_SHELL_ENV=1`
- `DAINEL_SHELL_ENV_TIMEOUT_MS=15000`
...
Dainel stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.dainel/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.dainel/credentials/oauth.json` (or `$DAINEL_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.dainel/agent/*` (migrated by `dainel doctor` into `~/.dainel/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `DAINEL_OAUTH_DIR`
- Agent dir (default agent root override): `DAINEL_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Dainel imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Dainel derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/dainel/dainel-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/dainel/dainel.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.dainel/credentials/whatsapp/biz
          // authDir: "~/.dainel/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `dainel doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "dainel", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `agentDir`: default `~/.dainel/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `dainel.json`).
...
Dainel starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
Dainel starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `dainel plugins install @dainel/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Dainel starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Dainel spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Dainel
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/dainel" } }
}
```
...
When a file exceeds this limit, Dainel logs a warning and injects a truncated
...
message envelopes). If unset, Dainel uses the host timezone at runtime.
...
`[dainel]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[dainel]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[dainel]"`, Dainel will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Dainel generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.dainel/settings/tts.json",
      elevenlabs: {
...
Dainel also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Dainel currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `dainel configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:dainel`: all built-in Dainel tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.dainel/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.dainel/sandboxes",
        docker: {
          ...
          apparmorProfile: "dainel-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "dainel-sandbox-browser:bookworm-slim",
          containerPrefix: "dainel-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Dainel uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.dainel/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Dainel config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Dainel writes/merges a `models.json` into
`~/.dainel/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Dainel uses
...
- Shortcut: `dainel onboard --auth-choice opencode-zen`.
...
Shortcut: `dainel onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `dainel onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `dainel onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `dainel onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `dainel onboard --auth-choice minimax-api`.
...
- Override the agent config root with `DAINEL_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.dainel/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.dainel/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.dainel/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Dainel stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.dainel/skills` (workspace skills
...
from `~/.dainel/extensions`, `<workspace>/.dainel/extensions`, plus any
...
Dainel can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Dainel.app menubar, or `dainel gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/dainel" }
...
- Examples: `"/ui"`, `"/dainel"`, `"/apps/dainel"`.
...
- When a connection comes from one of these IPs, Dainel uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `dainel gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `DAINEL_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `DAINEL_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Dainel
...
macOS app behavior:
- Dainel.app watches `~/.dainel/dainel.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.dainel/dainel.json` (or `DAINEL_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.dainel/dainel.json` (or `DAINEL_CONFIG_PATH`)
...
- `DAINEL_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `dainel --dev …` → uses `~/.dainel-dev` + shifts ports from base `19001`
- `dainel --profile <name> …` → uses `~/.dainel-<name>` (port via config/env/flags)
...
Example:
```bash
DAINEL_CONFIG_PATH=~/.dainel/a.json \
DAINEL_STATE_DIR=~/.dainel-a \
...
Gmail helper config (used by `dainel webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "dainel@gmail.com",
...
- Set `DAINEL_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Dainel defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__dainel/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__dainel__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.dainel/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.dainel/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.dainel/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_dainel-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_dainel-bridge._tcp` under `~/.dainel/dns/` using the standard discovery domain `dainel.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `dainel.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `dainel.internal` via that server
