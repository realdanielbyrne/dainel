...
## Quick check: `dainel security audit` (formerly `dainel security audit`)
...
```bash
dainel security audit
dainel security audit --deep
dainel security audit --fix

# (On older installs, the command is `dainel ...`.)
```
...
- Tighten local perms (`~/.dainel` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... *spicy*. Here’s how to not get pwned.

Dainel is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, Dainel also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.dainel/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.dainel/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.dainel/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.dainel/credentials/oauth.json`
...
`dainel security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${DAINEL_GATEWAY_PASSWORD}
```
...
Dainel stores session transcripts on disk under `~/.dainel/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.dainel` (see the audit section below). If you need
...
Dainel can refresh the skills list mid-session:
...
- If you install plugins from npm (`dainel plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.dainel/extensions/<pluginId>/` (or `$DAINEL_STATE_DIR/extensions/<pluginId>/`).
  - Dainel uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
dainel pairing approve <channel> <code>
```
...
By default, Dainel routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Dainel has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.dainel/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.dainel or your logs.”
...
4. **Re-run audit**
   - `dainel security audit --deep` and confirm the report is clean.
...
- `~/.dainel/dainel.json`: `600` (user read/write only)
- `~/.dainel`: `700` (user only)

`dainel doctor` can warn and offer to tighten these permissions.

### 0.4) Network exposure (bind + port + firewall)

The Gateway multiplexes **WebSocket + HTTP** on a single port:
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `DAINEL_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_dainel-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `DAINEL_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `dainel doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `DAINEL_GATEWAY_PASSWORD`).

Rotation checklist (token/password):
1. Generate/set a new secret (`gateway.auth.token` or `DAINEL_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Dainel
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Dainel verifies the identity by resolving the
...
- Dainel will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.dainel/` (or `$DAINEL_STATE_DIR/`) may contain secrets or private data:

- `dainel.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `dainel status --all` (pasteable, secrets redacted) over raw logs.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.dainel/sandboxes`
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `dainel gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `DAINEL_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/dainel/dainel-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.dainel/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Dainel version
...
Found a vulnerability in Dainel? Please report responsibly:
