---
summary: "First-run onboarding flow for Dainel (macOS app)"
read_when:
...
- Writes credentials to `~/.dainel/credentials/oauth.json`
...
The app can install the global `dainel` CLI via npm/pnpm so terminal
...
On the first agent run, Dainel bootstraps a workspace (default `~/clawd`):
...
```bash
dainel webhooks gmail setup --account you@gmail.com
```
