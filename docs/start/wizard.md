...
The onboarding wizard is the **recommended** way to set up Dainel on macOS,
...
`dainel dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
...
(`web_fetch` works without a key). Easiest path: `dainel configure --section web`
...
1) **Existing config detection**
   - If `~/.dainel/dainel.json` exists, choose **Keep / Modify / Reset**.
   - Re-running the wizard does **not** wipe anything unless you explicitly choose **Reset**
     (or pass `--reset`).
   - If the config is invalid or contains legacy keys, the wizard stops and asks
     you to run `dainel doctor` before continuing.
...
2) **Model/Auth**
   ...
   - **OpenAI API key**: uses `OPENAI_API_KEY` if present or prompts for a key, then saves it to `~/.dainel/.env` so launchd can read it.
  ...
  - OAuth credentials live in `~/.dainel/credentials/oauth.json`; auth profiles live in `~/.dainel/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
...
5) **Channels**
  ...
  - DM security: default is pairing. First DM sends a code; approve via `dainel pairing approve <channel> <code>` or use allowlists.
...
7) **Health check**
   - Starts the Gateway (if needed) and runs `dainel health`.
   - Tip: `dainel status --deep` adds gateway health probes to status output (requires a reachable gateway).
...
Use `dainel agents add <name>` to create a separate agent with its own workspace,
...
```bash
dainel onboard --non-interactive \
  --mode local \
  --auth-choice apiKey \
...
- Stores it under `~/.dainel/tools/signal-cli/<version>/`.
...
Typical fields in `~/.dainel/dainel.json`:
...
`dainel agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.dainel/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.dainel/agents/<agentId>/sessions/`.
