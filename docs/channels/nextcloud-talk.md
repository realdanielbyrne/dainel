...
```bash
dainel plugins install @dainel/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
dainel plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Dainel" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `dainel pairing list nextcloud-talk`
  - `dainel pairing approve nextcloud-talk <CODE>`
