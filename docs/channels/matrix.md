...
Matrix is an open, decentralized messaging protocol. Dainel connects as a Matrix **user**
...
```bash
dainel plugins install @dainel/matrix
```

Local checkout (when running from a git repo):

```bash
dainel plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `dainel plugins install @dainel/matrix`
   - From a local checkout: `dainel plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Dainel calls the same
     login endpoint, stores the access token in `~/.dainel/credentials/matrix/credentials.json`,
...
- On first connection, Dainel requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Dainel logs a warning.
...
`~/.dainel/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `dainel pairing list matrix`
  - `dainel pairing approve matrix <CODE>`
...
- On startup, Dainel resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
