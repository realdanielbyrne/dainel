...
```bash
dainel plugins install @dainel/msteams
```

Local checkout (when running from a git repo):
```bash
dainel plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Dainel will offer the local install path automatically.
...
3) Configure Dainel with those credentials.
...
## Goals
- Talk to Dainel via Teams DMs, group chats, or channels.
...
- On startup, Dainel resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.dainel/dainel.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Dainel, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `dainel-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `dainel plugins install @dainel/msteams`
   - From a local checkout: `dainel plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Dainel in Teams", "full": "Dainel in Teams" },
...
Dainel handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Dainel only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/DainelShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
Dainel sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `dainel message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.dainel/msteams-polls.json`.
...
**CLI:**
```bash
dainel message send --channel msteams \
...
dainel message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
dainel message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
dainel message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
dainel message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
