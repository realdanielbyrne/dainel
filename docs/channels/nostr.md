...
read_when:
  - You want Dainel to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Dainel to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`dainel onboard`) and `dainel channels add` list optional channel plugins.
...
```bash
dainel plugins install @dainel/nostr
```

Use a local checkout (dev workflows):

```bash
dainel plugins install --link <path-to-dainel>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "dainel@example.com",
        "lud16": "dainel@example.com"
      }
    }
  }
}
