...
read_when:
  - Setting up Twitch chat integration for Dainel
---
# Twitch (plugin)

Twitch chat support via IRC connection. Dainel connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
dainel plugins install @dainel/twitch
```

Local checkout (when running from a git repo):

```bash
dainel plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `DAINEL_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "dainel",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use DAINEL_TWITCH_ACCESS_TOKEN env var)
...
```bash
DAINEL_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
