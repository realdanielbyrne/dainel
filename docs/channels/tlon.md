...
Tlon is a decentralized messenger built on Urbit. Dainel connects to your Urbit ship and can
...
```bash
dainel plugins install @dainel/tlon
```

Local checkout (when running from a git repo):

```bash
dainel plugins install ./extensions/tlon
```
...
Use these with `dainel message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Dainel replies in-thread.
